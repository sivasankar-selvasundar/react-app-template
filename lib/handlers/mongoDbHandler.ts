...
export class MongoDbHandler implements IHandler {
  async handleRequest(requestBody: any): Promise<any> {
    ...
    switch (requestBody.operation) {
      case 'READ':
        response = await db
          .collection(requestBody.collectionName)
          .find({ ...JSON.parse(requestBody.filter) })
          .toArray();
        break;
      case 'CREATE':
        response = (await db.collection(requestBody.collectionName).insertOne(JSON.parse(requestBody.data)))
          .acknowledged;
        break;
    }
